...
async def test_pds_group_uses_one_native_sglang_batch(monkeypatch):
        ...
        def generate_request(self, request, _):
            async def responses():
                yield [
                    ...
                    {"output_ids": [7], "meta_info": _pds_meta(0.3)},
                ]
    ...
    outputs = await server._generate_group_local([request([1, 2], 1.0), request([3, 4], 2.0)])
    ...
    assert outputs[0].log_probs == outputs[1].log_probs == pytest.approx([math.log(0.5)])
